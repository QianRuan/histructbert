...
class LASentAddEmb(nn.Module):
    ...
    def __init__(self, config, args, max_nsent):
        ...
        if(self.args.sent_se_comb_mode == 'concat'):
            self.a_position_embeddings = nn.Embedding(config.max_position_embeddings, int(config.hidden_size/2))
            self.b_position_embeddings = nn.Embedding(config.max_position_embeddings, int(config.hidden_size/2))
        else:
