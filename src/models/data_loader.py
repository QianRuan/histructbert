...
class Batch(object):
    ...
    def _pad2(self, data, pad_id, width=-1):
        if (width == -1):
            width = max(len(d) for d in data)
        rtn_data = [d + [(pad_id,pad_id)] * (width - len(d)) for d in data]
        return rtn_data
    
    def _pad3(self, data, pad_id, width=-1):
        if (width == -1):
            width = max(len(d) for d in data)
        rtn_data = [d + [(pad_id,pad_id,pad_id)] * (width - len(d)) for d in data]
        return rtn_data
    ...
    def __init__(self, data=None, device=None, is_test=False):
        ...
        if data is not None:
            ...
            pre_sent_struct_vec = [x[0] for x in data]#
...
##            pre_tgt = [x[3] for x in data]
            ...
            print('2-pre_section_names', len(pre_section_names),pre_section_names.shape,pre_section_names)
            ...
            print('3-section_names',section_names.shape,section_names)
...
##            mask_tgt = 1 - (tgt == 0)
            ...
            #print(self._pad(pre_clss, -1).shape)
...
class DataIterator(object):
    ...
    def preprocess(self, ex, is_test):
        ...
        print('0-pre_section_names', len(pre_section_names),pre_section_names.shape, pre_section_names)
...
#                sn = list(sn_emb.keys())
        ...
        print('1-section_names', len(section_names),section_names.shape, section_names)
